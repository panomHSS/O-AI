...
O-AI 0.6.2A introduces Alembic revision `0001_v061_baseline` for the verified O-AI 0.6.1 SQLite schema. Application startup opens the configured SQLite database in read-only mode and verifies the current managed schema and revision; it never runs Alembic migrations, creates tables, stamps revisions, or writes backups.
...
## Citation persistence (0.6.3)

Revision `0002_message_citations` adds immutable evidence snapshots for assistant answers generated by Knowledge Intelligence. Each stored citation records its display order, source identifiers and metadata, excerpt and SHA-256 excerpt hash, confidence, and evidence type. Citations are deleted automatically when their assistant message is deleted; snapshot fields deliberately do not depend on the current document row.

## Personal Memory (0.7.0A)
