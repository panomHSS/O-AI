...
def test_d101_update_proposal_wiring_composes_existing_d73_and_d101_store() -> None:
    service = get_calendar_update_followup_proposal_service(
        approval_service=get_calendar_write_approval_service(),
    )

    assert isinstance(service, CalendarUpdateFollowupProposalService)


def test_d101_update_proposal_wiring_adds_no_execution_or_provider_dependency() -> None:
    source = inspect.getsource(get_calendar_update_followup_proposal_service)
