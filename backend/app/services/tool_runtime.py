"""Authorization-gated D38 Tool runtime."""
...
class ToolRuntime:
    ...
    def execute(
        ...
        if not isinstance(authorization, ExecutionAuthorization):
            return self._failed(request.request_id, "tool_authorization_rejected")
        ...
        if (
            ...
            return self._failed(request.request_id, "request_authorization_mismatch")

        if request.command != "tool.execute":
            return self._failed(request.request_id, "tool_command_rejected")

        if plan.owner_approval_required:
            return self._failed(request.request_id, "tool_plan_invalid")
        ...
        if adapter is None:
            return self._failed(request.request_id, "tool_adapter_unavailable")

        try:
            result = adapter.execute(request, plan)
        except Exception:
            return self._failed(request.request_id, "tool_execution_failed")

        if not self._valid_result(request, result):
            return self._failed(request.request_id, "tool_result_invalid")
