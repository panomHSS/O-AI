...
class ChatService:
    ...
    def send_message(self, message: str, recent_messages: Sequence[ChatContextMessage] = (), memories: Sequence[ResolvedMemory] = ()) -> str:
        return self._provider.generate_reply(self._format_provider_input(message, recent_messages, memories))

    @staticmethod
    def _format_provider_input(message: str, recent_messages: Sequence[ChatContextMessage], memories: Sequence[ResolvedMemory]) -> str:
        blocks: list[str] = []
        if recent_messages:
