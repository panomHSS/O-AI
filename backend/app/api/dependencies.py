...
@lru_cache
def get_google_calendar_update_module_adapter() -> GoogleCalendarUpdateModuleAdapter:
    return GoogleCalendarUpdateModuleAdapter(
        credential_broker=get_credential_access_broker(),
        client=get_google_calendar_write_client(),
    )
...
@lru_cache
def get_google_calendar_delete_module_adapter() -> GoogleCalendarDeleteModuleAdapter:
    return GoogleCalendarDeleteModuleAdapter(
        credential_broker=get_credential_access_broker(),
        client=get_google_calendar_write_client(),
    )
...
@lru_cache
def get_calendar_update_delete_private_registry() -> AdapterRegistry:
    return AdapterRegistry(
        (
...
@lru_cache
def get_calendar_update_delete_private_permission_policy() -> CapabilityPermissionPolicy:
    registry = get_calendar_update_delete_private_registry()
    return CapabilityPermissionPolicy(
        registry=registry,
        permissions=(
            ExecutableCapabilityPermission(
...
def get_calendar_update_delete_private_guard(
    audit: ExecutionAuditTrail = Depends(get_execution_audit_trail),
) -> ExecutionGuard:
    return ExecutionGuard(
        registry=get_calendar_update_delete_private_registry(),
        permission_policy=get_calendar_update_delete_private_permission_policy(),
        audit=audit,
    )


def get_calendar_update_delete_private_runtime(
    audit: ExecutionAuditTrail = Depends(get_execution_audit_trail),
) -> ModuleRuntime:
    return ModuleRuntime(
        registry=get_calendar_update_delete_private_registry(),
...
def get_calendar_update_delete_execution_service(
    guard: ExecutionGuard = Depends(get_calendar_update_delete_private_guard),
    runtime: ModuleRuntime = Depends(get_calendar_update_delete_private_runtime),
) -> CalendarUpdateDeleteExecutionService:
