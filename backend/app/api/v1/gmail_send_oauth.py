"""D88 local-owner Gmail Send OAuth credential control API.

This route manages only the isolated gmail.send credential subject. It grants
no D87 approval, D88 execution claim, ModuleRuntime authorization, or Gmail
...
router = APIRouter(
    prefix="/oauth/google-gmail-send",
    tags=["oauth"],
)

GMAIL_SEND_OAUTH_STATE_COOKIE = "oai_google_gmail_send_oauth_state"
...
def _status_response(
    ...
    *,
    ...
    return OAuthConnectionStatusResponse(
        connector_enabled=settings.oai_gmail_send_enabled,
...
@router.get("/start")
def start_google_gmail_send_oauth(
    service: Annotated[
        GoogleOAuthLifecycleService,
        Depends(get_google_gmail_send_oauth_lifecycle_service),
    ],
) -> RedirectResponse:
    try:
    ...
    response.set_cookie(
        ...
        secure=False,
        samesite="lax",
        path=GMAIL_SEND_OAUTH_CALLBACK_PATH,
    )
    return response
...
def _callback_redirect(
    ...
    *,
    ...
    # D88 adds no new frontend send UX. Reuse the bounded Gmail connection
    # destination while keeping the OAuth subject/cookie/callback fully separate.
    destination = (
        owner_ui_config.gmail_connected_url()
        if connected
        else owner_ui_config.gmail_error_url(
...
@router.get("/callback")
def google_gmail_send_oauth_callback(
    service: Annotated[
        GoogleOAuthLifecycleService,
        Depends(get_google_gmail_send_oauth_lifecycle_service),
    ],
    ...
    cookie_state: Annotated[
        str | None,
        Cookie(alias=GMAIL_SEND_OAUTH_STATE_COOKIE),
    ] = None,
) -> RedirectResponse:
    if (
...
def google_gmail_send_oauth_status(
    service: Annotated[
        GoogleOAuthLifecycleService,
        Depends(get_google_gmail_send_oauth_lifecycle_service),
    ],
    runtime_config: Annotated[
        ...
        Depends(get_google_gmail_send_oauth_runtime_config),
    ],
...
def disconnect_google_gmail_send_oauth(
    _: Annotated[
        None,
