"""D76 local-owner Gmail OAuth credential control API."""
...
from app.api.dependencies import (
    get_google_gmail_oauth_lifecycle_service,
    get_google_gmail_oauth_runtime_config,
...
router = APIRouter(
    prefix="/oauth/google-gmail",
    tags=["oauth"],
)

GMAIL_OAUTH_STATE_COOKIE = "oai_google_gmail_oauth_state"
GMAIL_OAUTH_CALLBACK_PATH = "/api/v1/oauth/google-gmail/callback"
...
def _status_response(
    ...
    *,
    ...
    return OAuthConnectionStatusResponse(
        connector_enabled=settings.oai_gmail_connector_enabled,
...
@router.get("/start")
def start_google_gmail_oauth(
    service: Annotated[
        GoogleOAuthLifecycleService,
        Depends(get_google_gmail_oauth_lifecycle_service),
    ],
) -> RedirectResponse:
    try:
...
def google_gmail_oauth_callback(
    ...
    cookie_state: Annotated[
        str | None,
        Cookie(alias=GMAIL_OAUTH_STATE_COOKIE),
    ] = None,
) -> RedirectResponse:
    if (
...
def google_gmail_oauth_status(
    ...
    runtime_config: Annotated[
        ...
        Depends(get_google_gmail_oauth_runtime_config),
    ],
...
def disconnect_google_gmail_oauth(
    _: Annotated[
        None,
