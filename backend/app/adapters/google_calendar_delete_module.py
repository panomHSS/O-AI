"""D75 private ModuleAdapter for one approved Google Calendar delete."""
...
from app.contracts.google_calendar import (
    ...
    GOOGLE_CALENDAR_DELETE_CAPABILITY_NAME,
    GOOGLE_CALENDAR_DELETE_CREDENTIAL_PROFILE_ID,
...
from app.contracts.google_calendar_write import GOOGLE_CALENDAR_DELETE_EVENT_OPERATION
...
CALENDAR_DELETE_ERROR_CREDENTIAL_UNAVAILABLE = (
    "calendar_delete_credential_unavailable"
)


class GoogleCalendarDeleteModuleAdapter:
    """Execute exactly one authorized delete through D62 and D75."""
    ...
    def __init__(
        ...
        *,
        ...
        client: GoogleCalendarEventDeleter | None = None,
    ) -> None:
    ...
    def adapter_id(self) -> str:
        return GOOGLE_CALENDAR_DELETE_ADAPTER_ID

    @property
    def module_name(self) -> str:
        return GOOGLE_CALENDAR_DELETE_MODULE_NAME
    ...
    def execute(self, request: CommandRequest, plan: ExecutionPlan) -> Result:
        ...
        if (
            step.sequence != 1
            or step.operation != GOOGLE_CALENDAR_DELETE_EVENT_OPERATION
        ):
            return self._failed(request.request_id, "unsupported_operation")
        try:
            delete_request, write_digest = calendar_delete_request_from_parameters(
                step.parameters
            )
        except ValueError:
            return self._failed(
        ...
        if calendar_write_digest(delete_request) != write_digest:
            return self._failed(
                request.request_id,
                "calendar_delete_parameters_invalid",
            )
        ...
        if (
            credential.profile_id != GOOGLE_CALENDAR_DELETE_CREDENTIAL_PROFILE_ID
            or credential.plugin_id != GOOGLE_CALENDAR_PLUGIN_ID
            or credential.plugin_version != GOOGLE_CALENDAR_PLUGIN_VERSION
            or credential.capability_name != GOOGLE_CALENDAR_DELETE_CAPABILITY_NAME
        ...
        try:
            result = self._client.delete_event(
                credential.secret,
        ...
        if result.event_id != delete_request.target.event_id:
            return self._failed(
                request.request_id,
                GOOGLE_CALENDAR_DELETE_ERROR_INDETERMINATE,
            )
...
__all__ = [
    "CALENDAR_DELETE_ERROR_CREDENTIAL_UNAVAILABLE",
    "GoogleCalendarDeleteModuleAdapter",
]
